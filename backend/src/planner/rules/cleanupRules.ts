...
export const cleanupRules = (input: PlannerInput): RuleResult => {
  const steps = input.filtered.emails.flatMap<PartialPlanStep>((email) => {
    ...
    if (
      /(newsletter|digest|announcement|update|promo|promotion)/i.test(text) &&
      !/(deadline|interview|application)/i.test(text)
    ) {
      return [
        {
          ...
          reason:
            'Informational updates should be labeled for lower-noise triage.',
          confidence: 0.72,
        },
        {
          workflow: 'Inbox Cleanup',
          ...
          reason:
            'Low-actionability update can leave the inbox after labeling.',
          confidence: 0.67,
        },
      ];
    }
