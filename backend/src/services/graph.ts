...
export const listMessages = async (
  ...
  const params = new URLSearchParams({
    ...
    $select:
      'id,conversationId,subject,bodyPreview,receivedDateTime,importance,from,webLink',
  });
...
export const listMailFolders = async (accessToken: string) => {
  const response = await fetchWithTimeout(
    'https://graph.microsoft.com/v1.0/me/mailFolders?$select=id,displayName,wellKnownName',
    {
      headers: { Authorization: `Bearer ${accessToken}` },
    },
    env.aiTimeoutMs
