...
const parseAndValidate = async <S extends z.ZodTypeAny>(
  prompt: string,
  schema: S,
  requestMeta?: AiRequestMetadata
): Promise<z.infer<S>> => {
  let attempt = 0;
...
export const decideAgentActions = async (
  input: {
...
export const planAgentActions = async (
  input: {
    goals: Array<{ goal: string; weight: number }>;
    autopilotLevel: number;
    context: string;
...
export const reflectOnExecution = async (
  input: {
    goals: Array<{ goal: string; weight: number }>;
    context: string;
    plan: unknown;
...
export const generateStrategistAdjustments = async (
  input: {
    goals: Array<{ goal: string; weight: number }>;
...
export const generateActivityFeed = async (
  input: {
    goals: Array<{ goal: string; weight: number }>;
