...
export const draftReplyTool: ToolDefinition<Input, Output> = {
  ...
  execute: async (ctx: ToolContext) => {
    const emailResult = await query<{
      ...
      'SELECT subject, sender_name, sender_email, body_preview, thread_id FROM emails WHERE id = $1 AND user_id = $2',
    ...
    const draftResponse = await fetchWithTimeout(
      `https://graph.microsoft.com/v1.0/me/messages/${ctx.messageId}/createReply`,
      {
        method: 'POST',
