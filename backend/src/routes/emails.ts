...
emailsRouter.post('/sync', authMiddleware, async (req: AuthRequest, res) => {
  const userId = req.user?.userId;
  if (!userId) return res.status(401).json({ error: 'Unauthorized' });

  await ingestionQueue.add('sync-user', { userId }, { attempts: 3, backoff: { type: 'exponential', delay: 2000 } });
  return res.json({ status: 'queued' });
});

emailsRouter.get('/', authMiddleware, validate(listSchema, 'query'), async (req: AuthRequest, res) => {
  const userId = req.user?.userId;
  if (!userId) return res.status(401).json({ error: 'Unauthorized' });

  const { limit, offset, status, classification, query: queryText } =
