...
preferencesRouter.get('/', authMiddleware, async (req: AuthRequest, res) => {
...
preferencesRouter.put('/', authMiddleware, validate(preferencesSchema), async (req: AuthRequest, res) => {
  const userId = req.user?.userId;
  if (!userId) return res.status(401).json({ error: 'Unauthorized' });
