...
webhooksRouter.post('/graph', async (req, res) => {
  ...
  for (const notification of notifications) {
    ...
    if (!row) continue;
    if (notification.clientState && notification.clientState !== row.client_state) continue;

    await ingestionQueue.add('sync-user', { userId: row.user_id }, { attempts: 3, backoff: { type: 'exponential', delay: 2000 } });
  }
