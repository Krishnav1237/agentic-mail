...
agentRouter.put(
  '/goals',
  authMiddleware,
  validate(goalsSchema),
  asyncRoute(async (req: AuthRequest, res) => {
    const userId = req.user?.userId;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });

    const { goals, autopilotLevel, personalityMode } = req.body as z.infer<
      typeof goalsSchema
    >;
...
agentRouter.post(
  ...
  asyncRoute(async (req: AuthRequest, res) => {
    ...
    const { actionId, status, notes, metadata } = req.body as z.infer<
...
agentRouter.get(
  ...
  asyncRoute(async (req: AuthRequest, res) => {
    ...
    const listResult = await query<AgentActionRow>(
    ...
    return res.json({
      ...
      limit,
      offset,
      ...summarizeActions(listResult.rows),
    });
  })
);
...
agentRouter.post(
  ...
  asyncRoute(async (req: AuthRequest, res) => {
    ...
    const { intents, sessionOverrides, priorityBoosts, sessionId, ttlHours } =
      req.body as z.infer<typeof intentSchema>;

    const state = await updateIntentState({
      userId,
      intents,
      sessionOverrides,
      priorityBoosts,
...
agentRouter.post(
  ...
  asyncRoute(async (req: AuthRequest, res) => {
    ...
    const { actionId, payloadOverride } = req.body as z.infer<
...
agentRouter.post(
  ...
  asyncRoute(async (req: AuthRequest, res) => {
    ...
    >;
    if (!payloadOverride)
      return res.status(400).json({ error: 'Missing payloadOverride' });
...
agentRouter.post(
  '/preview/approve-all',
  authMiddleware,
  validate(workflowPreviewSchema),
  asyncRoute(async (req: AuthRequest, res) => {
    const userId = req.user?.userId;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
