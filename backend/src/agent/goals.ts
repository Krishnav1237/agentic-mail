...
const normalizeGoals = (goals: RawGoal[] | null | undefined): Array<{ goal: string; weight: number }> => {
  if (!goals) return [];
...
export const getUserGoals = async (userId: string): Promise<AgentGoalState> => {
  const result = await query<{ goals: RawGoal[]; autopilot_level: number; personality_mode: string | null }>(
  ...
  return {
    ...
    personalityMode: (row.personality_mode ?? 'proactive') as 'chill' | 'proactive' | 'aggressive'
  };
};

export const updateUserGoals = async (
  userId: string,
  input: { goals: Array<{ goal: string; weight: number }>; autopilotLevel: 0 | 1 | 2; personalityMode?: 'chill' | 'proactive' | 'aggressive' }
) => {
  await query(
    ...
    [userId, JSON.stringify(input.goals), input.autopilotLevel, input.personalityMode ?? 'proactive']
  );
};
