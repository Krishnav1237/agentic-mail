...
const buildAccuracyMap = async (userId: string) => {
  const result = await query<{ action_type: string; status: string; count: number }>(
  ...
  for (const row of result.rows) {
    if (!grouped[row.action_type]) grouped[row.action_type] = { positive: 0, negative: 0 };
    if (['approved', 'accepted', 'always_allow', 'executed'].includes(row.status)) {
...
export const getHistoricalAccuracy = async (userId: string, actionType: string) => {
  const cached = await getMemory<AccuracyCache>(userId, 'short', 'confidence_accuracy');
  ...
  await upsertMemory(userId, 'short', 'confidence_accuracy', { updatedAt, values });
...
const getRecencyWeight = async (userId: string, actionType: string) => {
  ...
  const diffDays = (Date.now() - new Date(lastTime).getTime()) / (1000 * 60 * 60 * 24);
...
const getContextSimilarity = async (userId: string, actionType: string, emailId?: string | null) => {
  if (!emailId) return 0.85;
  const currentEmail = await query<{ sender_email: string | null; subject: string | null }>(
    'SELECT sender_email, subject FROM emails WHERE id = $1 AND user_id = $2',
    [emailId, userId]
  );
  const senderDomain = extractDomain(currentEmail.rows[0]?.sender_email ?? null);
  const subject = (currentEmail.rows[0]?.subject ?? '').toLowerCase();

  if (!senderDomain) return 0.85;

  const recent = await query<{ sender_email: string | null; subject: string | null }>(
  ...
  const subjectTokens = new Set(subject.split(/\s+/).filter((token: string) => token.length > 3));
  for (const row of recent.rows) {
    ...
    if (domain && domain === senderDomain) {
      ...
      const overlap = otherSubject.split(/\s+/).filter((token: string) => token.length > 3 && subjectTokens.has(token));
...
export const getConfidenceFactors = async (input: {
  ...
  const historicalAccuracy = await getHistoricalAccuracy(input.userId, input.actionType);
  const recencyWeight = await getRecencyWeight(input.userId, input.actionType);
  const contextSimilarity = await getContextSimilarity(input.userId, input.actionType, input.emailId);
