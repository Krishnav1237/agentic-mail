...
const removeNotifications = async (userId: string, taskId: string) => {
  await query(`DELETE FROM notifications WHERE user_id = $1 AND task_id = $2`, [
    userId,
    taskId,
  ]);
};
...
const undoCreateTask = async (userId: string, payload: Record<string, any>) => {
  ...
  await query(`DELETE FROM extracted_tasks WHERE id = $1 AND user_id = $2`, [
    taskId,
    userId,
  ]);
...
const undoCreateCalendarEvent = async (
  userId: string,
...
const undoMarkImportant = async (userId: string, messageId: string | null) => {
  ...
  const auth = await getAuthContext(userId);
  if (auth.provider === 'google') {
    await modifyGmailMessage(auth.accessToken, messageId, {
      removeLabelIds: ['IMPORTANT', 'STARRED'],
    });
  } else {
    await fetchWithTimeout(
...
const undoMoveToFolder = async (
  ...
  if (!messageId) throw new Error('Missing message context');
  const auth = await getAuthContext(userId);
  const previousFolder = normalizeFolderName(
    String(payload?.previous_folder ?? 'inbox')
  );
  if (auth.provider === 'google') {
...
const undoLabelEmail = async (
  userId: string,
  payload: Record<string, any>,
  messageId: string | null
) => {
  if (!messageId) throw new Error('Missing message context');
  const label = normalizeLabelName(
    String(payload?.label ?? payload?.result?.label ?? '')
  ).trim();
  ...
  const categories = (existing?.categories ?? []).filter(
...
export const undoAction = async (input: {
  userId: string;
  actionId: string;
}) => {
  ...
  switch (action.action_type) {
    ...
    case 'create_calendar_event':
      result = await undoCreateCalendarEvent(
        input.userId,
        action.action_payload ?? {}
      );
      break;
    case 'snooze':
      result = await undoSnooze(
        input.userId,
        action.action_payload ?? {},
        action.email_id
      );
...
export const rollbackWorkflow = async (input: {
  userId: string;
  workflowId: string;
}) => {
...
export const detectRiskyOutcome = async (input: {
  ...
  if (
    input.actionType === 'send_reply' ||
    input.actionType === 'delete_email'
  ) {
    await logAgentStep({
      userId: input.userId,
      step: 'risky_action',
      message:
        input.actionType === 'send_reply'
          ? 'Send reply is irreversible'
          : 'Delete email is high-risk mailbox cleanup',
  ...
  if ((input.result as any)?.error) {
    await logAgentStep({
      ...
      data: {
        ...
        error: (input.result as any).error,
      },
    });
