...
const stateKey = (userId: string, loopType: string) =>
  `agent-state:${STATE_VERSION}:${userId}:${loopType}`;
...
const trimPreview = (value?: string) =>
  (value ?? '').replace(/\s+/g, ' ').trim().toLowerCase().slice(0, 160);
const normalizeSender = (value: string) =>
  value.replace(/\s+/g, ' ').trim().toLowerCase();
...
const normalizeThreads = (context: FilteredPlannerContext) => {
  ...
  for (const email of context.emails) {
    ...
    const threadKey =
      email.threadId || hashString(fallbackThreadKey).slice(0, 12);
    ...
    if (email.senderDomain)
      entry.senderDomains.add(email.senderDomain.toLowerCase());
    if (email.importance) entry.importance.add(email.importance.toLowerCase());
    if (email.classification)
      entry.classifications.add(email.classification.toLowerCase());
...
export const buildDecisionState = (input: {
  ...
  const recentActions = input.recentActions
    ...
    .sort((a, b) =>
      `${a.workflow}:${a.actionType}:${a.status}`.localeCompare(
        `${b.workflow}:${b.actionType}:${b.status}`
      )
    );
  ...
  return stableSort({
    ...
    goals: {
      goals: input.goals.goals
        .map((goal) => ({
          goal: goal.goal.trim().toLowerCase(),
          weight: Number(goal.weight.toFixed(2)),
        }))
    ...
    intents: {
      intents: [...input.intents.intents]
        .map((value) => value.trim().toLowerCase())
        .sort(),
      sessionOverrides: [...input.intents.sessionOverrides]
        .map((value) => value.trim().toLowerCase())
        .sort(),
...
export const getStoredStateHash = async (userId: string, loopType: string) => {
  const raw = await cacheRedis.get(stateKey(userId, loopType));
  if (!raw) return null;
