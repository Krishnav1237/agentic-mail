...
const defaultPolicy: PolicyState = { tools: {} };
...
export const recordAlwaysAllow = async (userId: string, actionType: string) => {
...
export const isAlwaysAllowed = async (userId: string, actionType: string) => {
  const policy = await getPolicyState(userId);
  return policy.tools[actionType]?.always_allow ?? false;
};
