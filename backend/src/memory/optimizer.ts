...
const referencesActiveSignal = (payload: unknown, activeTokens: Set<string>) => {
...
export const optimizeMemory = async (userId: string) => {
  const lastRun = await getMemory<string>(userId, 'short', 'memory_optimizer_last_run');
  if (!shouldRun(lastRun)) {
    return { optimized: false, reason: 'recent_run' };
  }

  const [openTasks, previewActions, recentSuccessfulActions, oldEpisodes] = await Promise.all([
    query<{ id: string }>(`SELECT id FROM extracted_tasks WHERE user_id = $1 AND status = 'open'`, [userId]),
    ...
    query<{ id: string; context: Record<string, unknown>; outcome: Record<string, unknown>; created_at: string }>(
  ...
  const eligibleEpisodes = oldEpisodes.rows.filter((episode) =>
  ...
  if (eligibleEpisodes.length > 0) {
    const currentDigest = (await getMemory<any[]>(userId, 'long', 'episodic_digest')) ?? [];
    const summary = {
    ...
    await upsertMemory(userId, 'long', 'episodic_digest', [summary, ...currentDigest].slice(0, 20));
    await query(
      `DELETE FROM episodic_memory WHERE id = ANY($1::uuid[])`,
      [eligibleEpisodes.map((episode) => episode.id)]
    );
  }

  const accuracy = await getMemory<AccuracyCache>(userId, 'short', 'confidence_accuracy');
  if (accuracy?.values) {
    const activeActionTypes = new Set(recentSuccessfulActions.rows.map((row) => row.action_type));
    const nextValues = Object.entries(accuracy.values).reduce<Record<string, number>>((acc, [actionType, value]) => {
  ...
  await upsertMemory(userId, 'short', 'memory_optimizer_last_run', new Date().toISOString());
