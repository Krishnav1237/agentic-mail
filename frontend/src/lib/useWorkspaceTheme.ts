...
/**
 * Tracks the app-wide light/dark preference for the Obligo workspace.
 *
 ...
 *
 * While mounted, the workspace also stamps `.obligo-workspace` on `<html>` so the
...
export function useWorkspaceTheme(): 'dark' | 'light' {
  ...
  useEffect(() => {
    ...
    root.classList.add('obligo-workspace');
    ...
    return () => {
      observer.disconnect();
      root.classList.remove('obligo-workspace');
    };
  }, []);
