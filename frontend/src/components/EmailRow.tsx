      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_180px_160px_auto] xl:items-center">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={clsx('badge', getCategoryTone(email.classification))}
            >
              {email.classification ?? 'other'}
            </span>
            {email.status && (
              <span className={clsx('badge', getStatusTone(email.status))}>
                {email.status}
              </span>
            )}
          </div>
        ...
        <div className="surface-subtle py-3">
          ...
          <div className="mt-2 flex items-center gap-2 text-sm font-semibold text-neutral-100">
            ...
            {email.ai_score !== null
              ? Number(email.ai_score).toFixed(2)
              : 'Pending'}
          </div>
          <div className="mt-2 h-2 rounded-full bg-neutral-900">
            <div
              ...
              style={{
                width: `${Math.max(Math.min((Number(email.ai_score ?? 0) / 4) * 100, 100), 8)}%`,
              }}
            />
          </div>
        </div>
