    ...
    <div className="w-full h-[450px] bg-black rounded-lg border border-white/5 flex flex-col font-mono relative overflow-hidden">
      ...
      <div className="h-10 border-b border-white/5 flex items-center justify-between px-4 bg-white/[0.02]">
        <div className="text-[10px] uppercase tracking-widest text-neutral-500">
          coreLoop.ts - Execution Trace
        </div>
        <div className="text-[10px] uppercase tracking-widest flex items-center gap-2">
          <div
            className={`w-2 h-2 rounded-full ${step === 0 ? 'bg-yellow-500 animate-pulse' : step === 1 ? 'bg-blue-500 animate-pulse' : 'bg-emerald-500'}`}
          />
          <span className="text-neutral-400">
            {step === 0
              ? 'BullMQ Ingesting'
              : step === 1
                ? 'HeavyPlanner Executing'
                : 'Pending Approval'}
          </span>
        </div>
      </div>
          ...
          <AnimatePresence mode="wait">
            {step === 0 && (
              <motion.div
                ...
                className="space-y-4"
              >
                <div className="text-neutral-500 mb-6 font-semibold">
                  [stage: PERCEIVE_AND_FILTER]
                </div>
                <div className="border border-white/10 p-4 rounded-md bg-white/[0.02] text-neutral-300">
                  ...
                  CS-341: "Updated Final Project Syllabus"
                </div>
                <div className="border border-white/10 p-4 rounded-md bg-white/[0.02] text-neutral-300">
                  <span className="text-amber-400 font-bold">[SYNC]</span>{' '}
                  Greenhouse: "Action Required - SWE Summer 2025"
                </div>
                <div className="border border-white/10 p-4 rounded-md bg-white/[0.02] text-neutral-300">
                  <span className="text-amber-400 font-bold">[SYNC]</span>{' '}
                  College Bookstore: "Save 15% on textbook rentals"
                </div>
              </motion.div>
            )}
            ...
            {step === 1 && (
              ...
              >
                <div className="text-neutral-500 mb-6 font-semibold">
                  [stage: NORMALIZE_AND_PLAN]
                </div>

                <motion.div
                  initial={{ x: -10 }}
                  animate={{ x: 0 }}
                  className="border-l-2 border-indigo-500 pl-4 py-2 text-neutral-300"
                >
                  <span className="text-neutral-500">Payload 1:</span> CS-341:
                  Updated Syllabus.
                  <br />
                  <span className="text-emerald-400">
                    {'> '} TOOL CALL:
                  </span>{' '}
                  create_task(title: 'Review CS-341 Specs')
                </motion.div>
                ...
                <motion.div
                  ...
                  className="border-l-2 border-indigo-500 pl-4 py-2 text-neutral-300 mt-4"
                >
                  <span className="text-neutral-500">Payload 2:</span>{' '}
                  Greenhouse SWE Summer 2025.
                  <br />
                  <span className="text-emerald-400">
                    {'> '} PIPELINE EVENT:
                  </span>{' '}
                  classify_opportunity('Internship: SWE')
                  <br />
                  <span className="text-blue-400">{'> '} TOOL CALL:</span>{' '}
                  draft_reply('Confirming availability for screening.')
                </motion.div>
                ...
                <motion.div
                  ...
                  className="border-l-2 border-indigo-500 pl-4 py-2 text-neutral-300 mt-4"
                >
                  <span className="text-neutral-500">Payload 3:</span> Bookstore
                  Promo.
                  <br />
                  <span className="text-amber-400">
                    {'> '} FAST_PLANNER:
                  </span>{' '}
                  Match rule 'Promotional'
                  <br />
                  <span className="text-red-400">{'> '} TOOL CALL:</span>{' '}
                  archive_email(id) - *Idempotent safe*
                </motion.div>
              </motion.div>
            )}
              ...
              >
                ...
                <div className="rounded border border-indigo-500/30 bg-indigo-500/10 p-4 mb-4 text-left font-sans shadow-lg max-w-sm">
                  ...
                  <div className="text-sm text-neutral-200">
                    The agent drafted a reply to the SWE Summer 2025 recruiter.
                    This is a high-risk tool (`send_reply`) and requires preview
                    execution.
                  </div>
                </div>
                <div className="text-neutral-500 mt-4">
                  1 Task created · 1 Draft ready · 1 Thread archived
                  <br />
                  <span className="text-[10px] opacity-50 mt-2 block">
                    state_hash updated via redis_client
                  </span>
                </div>
              </motion.div>
            )}
