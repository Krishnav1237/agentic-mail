    ...
    <div className="space-y-6">
        ...
        stats={[
          {
            ...
            helper: 'Items still waiting on you',
          },
          {
            label: 'Actions visible',
            value: String(actions.length),
            helper: 'Current page of execution history',
          },
          {
            label: 'Total actions',
            value: String(total),
            helper: 'Server-paginated audit trail',
          },
          {
            label: 'Daily summary',
            value: feed?.summary_date ?? 'Not yet',
            helper: 'Latest activity feed snapshot',
          },
        ]}
      />
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="glass-card rounded-xl p-5">
          ...
          {feed ? (
            <div className="mt-4 space-y-4">
              ...
              <div className="grid gap-3 md:grid-cols-3">
                ...
                <div className="surface-subtle">
                  ...
                  <div className="mt-3 text-sm leading-7 text-neutral-400 font-light">
                    {feed.summary.improvements?.join(', ') ||
            ...
            <p className="mt-4 text-sm leading-7 text-neutral-300">
              No activity feed yet. It will appear once the planner completes
              live work.
            </p>
          )}
        </div>
        ...
        <div className="surface-card">
          ...
          <p className="mt-4 text-sm leading-7 text-neutral-400 font-light">
            High-confidence safe actions can execute automatically depending on
            autopilot level. Riskier steps stay here for review, keeping the
            agent proactive without becoming opaque.
          </p>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span
                          className={`badge ${getStatusTone(action.status)}`}
                        >
                          {action.status}
                        </span>
                        <span className="status-pill normal-case tracking-normal">
                          {action.workflow_name ?? 'Workflow bundle'}
                        </span>
                      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          message="Once the planner begins working through email and daily plans, the history and approval trail will show up here."
        />
      ) : (
        <div className="space-y-3">
          {actions.map((action) => (
            <div
              key={action.id}
              className="glass-card rounded-xl px-4 py-4 md:px-5"
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={`badge ${getStatusTone(action.status)}`}>
                      {action.status}
                    </span>
