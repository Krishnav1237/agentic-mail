...
export default function TasksPage() {
  ...
  const sort = (params.get('sort') ?? 'priority') as 'priority' | 'due' | 'created';
  ...
  }, [hasToken, limit, offset, status, category, query, sort, minPriority, maxPriority, setStatus]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        stats={[
          { label: 'Total results', value: String(total), helper: 'Server-paginated task inventory' },
          { label: 'Open on page', value: String(stats.openCount), helper: 'Currently actionable rows' },
          { label: 'With deadlines', value: String(stats.withDeadline), helper: 'Rows carrying due dates' },
          { label: 'Avg. priority', value: stats.averagePriority, helper: 'Visible page average' }
        ]}
      />
      ...
      <div className="glass-card rounded-[28px] p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            className="btn-ghost"
            onClick={() => setParams({ status: 'open', sort: 'priority', limit: String(limit), offset: '0' })}
          >
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-[2fr_repeat(5,minmax(0,1fr))]">
          ...
          <div>
            ...
            <select
              className="form-select mt-2"
              value={category}
              onChange={(event) => patchParams({ category: event.target.value })}
            >
          ...
          <div>
            ...
            <input
              className="form-input mt-2"
              value={minPriority}
              onChange={(event) => patchParams({ minPriority: event.target.value })}
            />
          </div>
          <div>
            ...
            <input
              className="form-input mt-2"
              value={maxPriority}
              onChange={(event) => patchParams({ maxPriority: event.target.value })}
            />
          </div>
        </div>
      </div>
